 ...
 * 3 一般
 */
int checkType(Eigen::Vector3d eigenVector, Eigen::Matrix3d evecs)
{
  double value0 = g_scaleValue;
...
//虚拟数据测试
// 和点的密度无关，设为0.01或者0.001间距，最后依旧会有翘起的部分，而且翘起的角度对称 ，一样。
int main(int argc, char** argv) {


  ros::init(argc, argv, "showNDTVoxel");
  ros::NodeHandle nh;
  ...
  if(pcl::io::loadPCDFile(filename, *target_cloud)) {
...
//      cout << "evecs_\n" <<evecs_ <<"\n evals_ \n" <<evals_ <<endl ;
  ...
  cout << "number of marker: " << Markers.size() << endl;
  ...
  while (ros::ok())
  {
    ...
    pcl::toROSMsg(*target_cloud, PonitMap2);
